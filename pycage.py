...
class Buyer():
    ...
    # 打印买家信息
    def print_info(self):
        print("买家设定: %s, 年龄: %d, 准备资金: %.1f万, 预留月供: %d个月, 其它预留: %.1f万, 月供上限: %.1f万" %
    ...
    def _cal_limit(self, house):
        ...
        for p in price_list:
            house.price = p
            contract_price_list = generate_linspace(house.evaluation_price(),
                                                    house.lowest_price(),
                                                    -0.5)
            ...
            for cp in contract_price_list:
                ...
                # 资金允许且初始所需资金更少
                if (house.is_buyable_for_current_contract_price() and
                    prepared_money <= min_prepared_money):
...
class House():
    ...
    # 是否普通住房: 同时满足三个条件
    def is_normal(self):
        if (self.floor_area_ratio >= 1.0 and
            self.area < 140.0 and
            self.contract_price < self.max_normal_price()):
            return True
        else:
            return False
    ...
    def loan_from_fund(self):
        if self.buyer.loan_from_fund_is_wanted:
            lf = min(120.0 if self.buyer.is_first else 80.0,
                     self.contract_price * 0.8,
                     self.loan()) # 不超过总贷款额
    ...
    def deed_tax(self):
        ...
        if self.contract_price > self.lowest_price():
            return ((self.contract_price - self.value_added_tax()) *
                    deed_tax_rate)
        else:
        ...
        if self.original_price > 0:
            if self.contract_price > self.lowest_price():
                return ((self.contract_price -
                         ...
                         self.contract_price * 0.1) * 0.2)
            else:
                return ((self.lowest_price() / 1.05 -
                         self.original_price -
                         self.lowest_price() * 0.1) * 0.2)
        # 原值不可追溯: 全额1%
        else:
            if self.contract_price > self.lowest_price():
    ...
    def total_interest_bank(self):
        return (self.monthly_repayment_bank() * self.loan_bank_years() * 12 -
                self.loan_from_bank())


    # 公积金贷款总利息 (等额本息)
    def total_interest_fund(self):
        return (self.monthly_repayment_fund() * self.loan_fund_years() * 12 -
                self.loan_from_fund())
    ...
    def total_interest_equal_principal(self):
        return (self.total_interest_bank_equal_principal() +
                self.total_interest_fund_equal_principal())
    ...
    def is_buyable_for_current_contract_price(self):
        if (self.total_prepared_money() <= self.buyer.max_prepared_money and
           self.monthly_repayment() <= self.buyer.max_monthly_repayment):
           return True
        else:
            return False
    ...
    def cal_plans(self):
        ...
        contract_price_list = generate_linspace(self.lowest_price(),
    ...
    # 打印房屋信息
    def print_info(self):
        print("房屋类型: %s%s%s, 原值: %.1f万, 面积: %.1f平, 房龄: %d年, 估计月租: %.2f万" %
